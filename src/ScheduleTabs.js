...
import {FACILITY_ID, API_BASE_URL} from './config/constants';
...
const ScheduleTabs = ({ schedule,onAddMember,onTabChange }) => {
          ...
          <div className="px-4">
            ...
            <ul className="space-y-4 pt-4">
              ...
              schedule[selectedTab].members.length > 0 ? (
                 schedule[selectedTab].members.map((member) => member !== null && member.status === "Booked" && (
                  <li
                    key={member.id}
...
const MemberCard = ({ member, facilityId, calendarScheduleId,disciplineId }) => {
    const { firstname, lastname, email, checkedin, id,img_src } = member;
